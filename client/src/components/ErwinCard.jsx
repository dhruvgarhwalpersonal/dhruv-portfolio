    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          top          : 'clamp(10%, 38%, 38%)',
          right        : 'clamp(2%, 3%, 3%)',
          left         : 'clamp(2%, auto, auto)',
          width        : 'clamp(240px, 90vw, 370px)',
          maxWidth     : '95vw',
          zIndex       : 20,
          pointerEvents: 'none',
      ...
      >
        <div style={{
          ...
          <div key={lyricKey} style={{ padding:'9px 14px 7px', textAlign:'center', animation:'erwinIn 0.45s cubic-bezier(0.16,1,0.3,1) both' }}>
            <p style={{ fontFamily:"'JetBrains Mono', monospace", fontSize:'clamp(9px, 2vw, 12px)', letterSpacing:'1.8px', lineHeight:1.7, color:accent, textTransform:'uppercase', fontWeight:600, margin:0, textShadow:`0 0 8px ${accent}, 0 0 20px ${accent}cc, 0 0 40px ${accent}66`, wordBreak:'break-word', overflowWrap:'break-word', whiteSpace:'normal' }}>{lyric}</p>
          </div>
      ...
      {/* LEFT arrow */}
      <div className="erwin-arrow-left" onClick={goPrev} onTouchEnd={e=>{ e.preventDefault(); goPrev(); }} style={{ position:'fixed', left:0, top:0, width:'clamp(44px, 72px, 72px)', height:'100vh', zIndex:50, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', opacity:mouseX>=0&&mouseX<0.12?1:0, transition:'opacity 0.3s ease', pointerEvents:mouseX>=0&&mouseX<0.12?'auto':'none' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'center', width:'clamp(30px, 36px, 36px)', height:'clamp(42px, 52px, 52px)', borderRadius:'3px', background:'rgba(0,0,0,0.18)', backdropFilter:'blur(6px)', border:`1px solid ${accent}18`, color:`${accent}bb`, fontSize:'clamp(14px, 18px, 18px)', fontFamily:"'JetBrains Mono', monospace", userSelect:'none', transition:'background 0.2s, border-color 0.2s, color 0.2s' }}
      ...
      {/* RIGHT arrow */}
      <div className="erwin-arrow-right" onClick={goNext} onTouchEnd={e=>{ e.preventDefault(); goNext(); }} style={{ position:'fixed', right:0, top:0, width:'clamp(44px, 72px, 72px)', height:'100vh', zIndex:50, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', opacity:mouseX>0.88?1:0, transition:'opacity 0.3s ease', pointerEvents:mouseX>0.88?'auto':'none' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'center', width:'clamp(30px, 36px, 36px)', height:'clamp(42px, 52px, 52px)', borderRadius:'3px', background:'rgba(0,0,0,0.18)', backdropFilter:'blur(6px)', border:`1px solid ${accent}18`, color:`${accent}bb`, fontSize:'clamp(14px, 18px, 18px)', fontFamily:"'JetBrains Mono', monospace", userSelect:'none', transition:'background 0.2s, border-color 0.2s, color 0.2s' }}
          onMouseEnter={e=>{ e.currentTarget.style.background='rgba(0,0,0,0.35)'; e.currentTarget.style.borderColor=`${accent}44`; e.currentTarget.style.color=accent; }}
          onMouseLeave={e=>{ e.currentTarget.style.background='rgba(0,0,0,0.18)'; e.currentTarget.style.borderColor=`${accent}18`; e.currentTarget.style.color=`${accent}bb`; }}
        >›</div>
      ...
      <style>{`
        ...
        /* ── Mobile: always show arrows via touch ── */
